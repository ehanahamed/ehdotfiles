...
# Modified version of volumectl from https://github.com/misterdanb/avizo
# this version uses image-path instead of image-resource
# image-path lets you use custom images
...
case "$dev_type" in
	sink)
		if [ "$muted" = 'true' ]; then
			image='volume_muted.png'
		elif [ "$volume" -le 33 ]; then
			image='volume_low.png'
		elif [ "$volume" -le 66 ]; then
			image='volume_medium.png'
		else
			image='volume_high.png'
		fi
	;;
	source)
		if [ "$muted" = 'true' ]; then
			...
			image='mic_unmuted.png'
		fi
	;;
esac

progress=$(echo "$volume" | awk '{ printf "%.2f", ($1 > 100 ? 1 : $1 / 100) }')
avizo-client --image-path="$image" --progress="$progress"
